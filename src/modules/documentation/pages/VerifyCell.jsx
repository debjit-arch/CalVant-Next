...
// // ── ✅ NEW: splits a flat feedback string into bullet points ──────────────────
// // Groq returns a single string like:
// //   "Missing: X, Y. Logic issue: Z. Present but weak: W. Recommendation: ..."
// // This splits on the known section markers so each shows as its own bullet.
// function splitFeedbackIntoBullets(fb) {
//   if (!fb) return [];
//   // Split before each known marker keyword that starts a new clause
//   const markers = ["Missing:", "Logic issue:", "Present but weak:", "Recommendation:", "Control gap:"];
//   const regex = new RegExp(`(?=${markers.map(m => m.replace(":", "\\:")).join("|")})`, "g");
//   return fb
//     .split(regex)
//     .map(s => s.trim().replace(/\.$/, "").trim()) // strip trailing dot
//     .filter(Boolean);
// }
...
//                 // ── ✅ NEW: bullet points only for policyStatements ────────
//                 const isPolicyStatements = key === "policyStatements";
//                 const bulletPoints = isPolicyStatements ? splitFeedbackIntoBullets(fb) : [];
...
//                     {/* ── ✅ NEW: policyStatements → bullet list; others → plain text ── */}
//                     {fb && (
//                       isPolicyStatements && bulletPoints.length > 1 ? (
//                         <ul style={{
//                           margin: "4px 0 0 0", paddingLeft: 16,
...
//                         </ul>
//                       ) : (
//                         <div style={{ fontSize: 11.5, color: "#6b7280", lineHeight: 1.5, marginTop: 4 }}>
//                           {fb}
//                         </div>
//                       )
//                     )}
