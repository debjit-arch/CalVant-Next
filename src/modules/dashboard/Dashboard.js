...
//               <ul>
//                 <li>
//                   <Link href="/risk-assessment" className="footer-link">
//                     Risk Management
//                   </Link>
//                 </li>
//                 <li>
...
import SprintoReplica from "./SprintoReplica";
import { useIsMobile } from "@/hooks/useIsMobile";
import { logPageLoad, logClick, MODULES } from "../admin/shell/services/activities"; // ← ADDED
...
const Dashboard = () => {
  ...
  const [frameworkNavOptions, setFrameworkNavOptions] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/framework/api/frameworks`)
      .then((res) => (res.ok ? res.json() : []))
      .then((data) => {
        console.log("frameworks data:", data[0]);
        setFrameworkNavOptions(
          data
            .filter((fw) => fw.label)
            .map((fw) => ({ label: fw.label, route: `/frameworks/${fw.id}` })),
  ...
  // ── ADDED: log page load once authenticated user is confirmed ──
  useEffect(() => {
    if (!user) return;
    logPageLoad("/", MODULES.DASHBOARD);
  }, [user]);
  ...
  // ── ADDED: handler for authenticated core module tile clicks ──
  const handleAuthTileClick = (label, route) => {
    logClick({ action: "navigate_module", module_name: label, destination: route }, MODULES.DASHBOARD);
    router.push(route);
  };
              ...
              {[
                ...
                ({
                  ...
                  <div
                    key={label}
                    className="dashboard-tile"
                    // ── CHANGED: was router.push(route), now logs before navigating ──
                    onClick={() => handleAuthTileClick(label, route)}
                    role="button"
                    tabIndex={0}
                    onKeyPress={(e) => e.key === "Enter" && handleAuthTileClick(label, route)}
                  >
                    <div className={`dashboard-tile-icon-wrapper ${bgColor}`}>
                      <Icon className={`w-12 h-12 ${iconColor}`} />
                    </div>
  ...
  // ============================================
  // GUEST USER VIEW — no logging (unauthenticated, no user identity)
  // ============================================
  return (
    <div className="dashboard-guest">
      {/* RESPONSIVE HEADER FOR GUESTS */}
