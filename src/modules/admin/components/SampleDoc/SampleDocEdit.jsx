...
import {
  ...
  CircularProgress, Snackbar, Alert, IconButton, Skeleton
} from '@mui/material'
...
const FormSkeleton = () => (
  <Stack spacing={4}>
    {[56, 120, 120, 120].map((h, i) => (
      <Skeleton key={i} variant="rounded" height={h} sx={{ borderRadius: '12px', bgcolor: '#F1F5F9' }} />
    ))}
  </Stack>
)
...
const SectionLabel = ({ children, dot }) => (
  <Stack direction="row" alignItems="center" spacing={1} mb={1}>
    <Box sx={{ w: 6, h: 6, width: 6, height: 6, borderRadius: '50%', bgcolor: dot }} />
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', flex: 1, minWidth: 100 }}>
          ...
          />
          {input.trim() && (
            <IconButton size="small" onClick={onAdd} sx={{ color: dot, bgcolor: bg, width: 22, height: 22, '&:hover': { bgcolor: bg } }}>
...
function SampleDocEdit() {
  ...
  const fetchDoc = async () => {
    ...
    try {
      ...
      const d = res.data
      setClause(d.clause || '')
      setType(Array.isArray(d.type) ? d.type : [])
      setDept(Array.isArray(d.dept) ? d.dept : [])
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#F8FAFC', p: { xs: 2, md: 4 } }}>
      ...
      <Paper
        elevation={0}
        sx={{
          maxWidth: 720, mx: 'auto', borderRadius: '20px',
