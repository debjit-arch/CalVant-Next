...
function BulkGap() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const file = new File([blob], 'bulk_gaps.csv', { type: 'text/csv' });

      const formData = new FormData();
      formData.append('file', file);

      // POST to the gap questions bulk upload endpoint
      await axios.post(
        'https://api.calvant.com/gap-questions/api/gaps/upload',
        formData,
        {
          headers: {
      ...
      setSnackbar({ 
        open: true, 
        message: 'Bulk gaps uploaded successfully ✅', 
        severity: 'success' 
      });
      ...
      >
        <StyledPaper elevation={0}>
          <Box sx={{ mb: 3 }}>
            ...
            >
              Upload and edit multiple gap questions at once via CSV file
            </Typography>
            ...
            >
              ...
              <Box>
                ...
                >
                  To add multiple <strong>Audit Questions</strong> or <strong>Departments</strong> in a single row, 
                ...
                >
                  Example: "Is training provided?; Is equipment calibrated?" or "IT; HR; Finance"
                </Typography>
              </Box>
            </Box>
          </Box>
