...
export default function ControlOwnershipAdmin() {
  ...
  const [allFetchedFrameworks, setAllFetchedFrameworks] = useState([]);
  const [controls, setControls] = useState([]);
  const [assignments, setAssignments] = useState([]);

  // Dynamically filter frameworks based on subscriptions in availableFrameworks
  const frameworks = useMemo(() => {
    const subscribedCodes = new Set((availableFrameworks || []).map((fw) => fw.code?.toUpperCase()));
    return allFetchedFrameworks.filter((fw) => subscribedCodes.has(fw.code?.toUpperCase()));
  }, [allFetchedFrameworks, availableFrameworks]);
  ...
  const handleSaveNotes = async () => {
    try {
      const updated = await api.updateOwnershipNotes(
        notesTarget.id,
        notesValue,
      );
      setAssignments((prev) =>
  ...
  const filtered = assignments.filter((a) => {
    if (filterFramework !== "ALL" && a.frameworkCode !== filterFramework)
      return false;
  ...
  const totalControls = [...new Set(assignments.map((a) => a.controlId))]
    .length;
  const totalFrameworks = [...new Set(assignments.map((a) => a.frameworkCode))]
    .length;
      ...
      >
        ...
        <Box sx={{ p: 3 }}>
            ...
            >
              {filtered.length === 0 ? (
                <Box sx={{ textAlign: "center", py: 6 }}>
                  <AssignmentInd
                    sx={{ fontSize: 48, color: "#e0e0e0", mb: 1 }}
                  />
                    ...
                    <TableBody>
                      {filtered.map((a) => (
                        <TableRow key={a.id} hover>
                          ...
                          <TableCell>
                            <RoleChip role={a.ownerRole} />
                          </TableCell>
                          <TableCell>
                            <Chip
                              label={a.frameworkCode}
                              size="small"
                              sx={{
                                fontWeight: 700,
                                fontSize: 11,
          ...
          <TabPanel value={tab} index={1}>
            {owners.length === 0 ? (
              <Alert severity="info">
                No users with process_owner or process_manager role found in
                your organization.
              </Alert>
            ) : (
              owners.map((owner) => {
                const ownerAssignments = assignments.filter(
                  (a) => a.ownerId === owner.id,
                );
                const frameworksOwned = [
                  ...new Set(ownerAssignments.map((a) => a.frameworkCode)),
                ];
                return (
                  <SectionCard
                    key={owner.id}
                    ...
                    action={
                      ...
                      >
                        {owner.role
                          ?.filter((r) =>
                            ["process_owner", "process_manager"].includes(r),
                          )
                          .map((r) => (
                            <RoleChip key={r} role={r} />
                          ))}
                        <Chip
                          label={`${ownerAssignments.length} controls`}
                          size="small"
                          sx={{
                            bgcolor: ACCENT + "12",
                            color: ACCENT,
                  ...
                  >
                    ...
                    {frameworksOwned.length === 0 ? (
                      ...
                      frameworksOwned.map((fw) => {
                        const fwControls = ownerAssignments.filter(
                          (a) => a.frameworkCode === fw,
                        );
                        return (
                          <Box key={fw} sx={{ mb: 2 }}>
                            <Box
                            ...
                            >
                              ...
                              <Typography fontSize={12} color="text.secondary">
                                {fwControls.length} control
                                {fwControls.length !== 1 ? "s" : ""}
                              </Typography>
                            </Box>
                            <Box
                              sx={{
          ...
          <TabPanel value={tab} index={2}>
            {frameworks.length === 0 ? (
              ...
              frameworks.map((fw) => {
                const fwAssignments = assignments.filter(
                  (a) => a.frameworkCode === fw.code,
                );
                const uniqueOwners = [
                  ...new Set(fwAssignments.map((a) => a.ownerId)),
                ].length;
                return (
                  <SectionCard
                    key={fw.id}
                    ...
                    action={
                      <Box sx={{ display: "flex", gap: 1 }}>
                        <Chip
                          label={`${fwAssignments.length} assigned`}
                          size="small"
                          sx={{
                            bgcolor: SUCCESS + "18",
                            color: SUCCESS,
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Box sx={{ display: "grid", gap: 2.5, mt: 1 }}>
            ...
            <FormControl size="small" fullWidth required>
              ...
              <Select
                value={assignForm.frameworkCode}
                label="1. Select Framework"
            ...
            >
              ...
              <Select
                value={assignForm.controlId}
                label={
                  loadingControls ? "Loading controls…" : "2. Select Control"
                }
                onChange={(e) =>
                  setAssignForm((f) => ({ ...f, controlId: e.target.value }))
                }
              >
                {controls.map((c) => (
              ...
              {loadingControls && <LinearProgressBar />}
            </FormControl>
            ...
            <FormControl size="small" fullWidth required>
              ...
              <Select
                ...
                label="3. Select Owner"
                onChange={(e) =>
                  setAssignForm((f) => ({ ...f, ownerId: e.target.value }))
                }
              >
                {owners.length === 0 ? (
