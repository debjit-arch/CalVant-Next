...
const TaskManagementDashboard = () => {
  ...
  const { isAdmin, userDeptNames, departmentLabel } = useMemo(() => {
    ...
    return {
      ...
      departmentLabel: isPrivilegedRole
        ? "All"
        : depts.map((d) => d.name).join(", ") || user?.department?.name || "General",
    };
  }, [user, isPrivilegedRole]);
  ...
  const loadTaskStats = useCallback(async () => {
    ...
    try {
      ...
      const orgTasks = tasks.filter((t) => t.organization === effectiveOrgId);
      const accessibleTasks = orgTasks.filter((t) => {
        if (isAdmin) return true;
        const taskDept = t.department?.trim().toLowerCase();
        return userDeptNames.includes(taskDept);
  ...
  // ── LOG: page load (once, after mount + user confirmed) ───────────────────
  useEffect(() => {
    if (!hasMounted || !user) return;
    logPageLoad("/task-management", MODULES.TASK);
  }, [hasMounted, user]);
  ...
  const pieData = [
    { name: "Approved",        value: taskStats.completed,      color: "#10b981", desc: `${taskStats.completed} approved tasks`  },
    { name: "Pending Approval",value: taskStats.pendingApproval,color: "#f59e0b", desc: `${taskStats.pendingApproval} pending review` },
    { name: "My Tasks",        value: taskStats.myTasks,         color: "#3b82f6", desc: `${taskStats.myTasks} assigned tasks`    },
  ].filter((d) => d.value > 0);
  ...
  const barData = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
    .map((name) => ({ name, value: realMonthlyData[name] || 0 }));
  ...
  const CustomBarTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const percentage = taskStats.total > 0
        ? ((data.value / taskStats.total) * 100).toFixed(1) : 0;
      const monthInfo = { Jan:"January",Feb:"February",Mar:"March",Apr:"April",May:"May",Jun:"June",Jul:"July",Aug:"August",Sep:"September",Oct:"October",Nov:"November",Dec:"December" };
      return (
        <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-lg min-w-[240px] max-h-[200px]">
          <div className="flex items-center gap-2 mb-2 pb-1">
            <div className="w-2 h-2 rounded-full bg-blue-500" />
  ...
  const statsCards = [
    { Icon: BarChart3,    value: taskStats.total,                         label: "Total",           color: "from-blue-400 to-blue-500"    },
    { Icon: Users,        value: taskStats.myTasks,                       label: "Scope Tasks",     color: "from-emerald-400 to-emerald-500"},
    { Icon: Clock,        value: taskStats.pendingApproval,               label: "Pending Approval",color: "from-orange-400 to-orange-500" },
    { Icon: CheckCircle2, value: taskStats.completed,                     label: "Approved Task",   color: "from-green-400 to-green-500"   },
    { Icon: Circle,       value: taskStats.total - taskStats.completed,   label: "Active",          color: "from-sky-400 to-sky-500"      },
  ];

  const actionCards = [
    { id: "tasks", icon: FileText, title: "Manage Tasks", subtitle: "All Tasks",  path: "/task-management/tasks",          color: "from-violet-400 to-violet-500" },
    { id: "dept",  icon: Users,    title: "My Tasks",     subtitle: "Repository", path: "/task-management/departmenttasks",color: "from-emerald-400 to-emerald-500",primary: true },
  ];

  const steps = [
    { target: "#dashboard-header", content: `Welcome to your ${departmentLabel} task management dashboard.` },
    { target: "#stats-grid",       content: "Quick metrics overview at a glance." },
    { target: "#charts-container", content: "Visual task distribution and trends analysis." },
    { target: "#action-cards",     content: "Quick access to all task management tools." },
  ];
          ...
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {departmentLabel} •{" "}
                  <span className="font-bold text-slate-900">{taskStats.total}</span>{" "}
                  total tasks
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                onClick={() => {
                  logClick({ action: "refresh_dashboard" }, MODULES.TASK, { url: "/task-management" });
                  loadTaskStats();
                }}
                title="Refresh"
                className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors border border-slate-200 flex items-center justify-center"
              ...
              <motion.button
                className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-semibold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2"
                onClick={() => {
                  logClick({ action: "open_guide" }, MODULES.TASK, { url: "/task-management" });
                  setRun(false);
                  setTimeout(() => setRun(true), 100);
                }}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-10 h-full">
          ...
          <div className="space-y-8 lg:space-y-10">
            ...
            >
              {statsCards.map(({ Icon, value, label, color }, i) => (
                <motion.div
                  ...
                  onClick={() => {
                    // LOG: stat card click
                    logClick({ filter: label.toLowerCase(), count: value }, MODULES.TASK, { url: "/task-management" });
                    router.push("/task-management/tasks");
                  }}
                  initial={hasMounted ? { opacity: 0, y: 20 } : false}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, delay: 0.15 + i * 0.05 }}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-lg lg:text-xl font-semibold text-slate-800 block leading-tight group-hover:text-slate-900">{value}</span>
                    <span className="text-xs lg:text-sm font-medium text-slate-600 uppercase tracking-wide">{label}</span>
                  </div>
                </motion.div>
              ))}
            </motion.section>
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 h-15">
                <AnimatePresence>
                  {actionCards.map(({ id, icon: Icon, title, subtitle, path, color, primary, show = true }, index) =>
                    show && (
                      <motion.div
                        key={id}
                        className={`group bg-white/70 backdrop-blur-sm border border-slate-100/50 rounded-xl p-4 h-full flex flex-col justify-between shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-white transition-all duration-300 cursor-pointer ${primary ? "ring-2 ring-emerald-200/50 bg-gradient-to-br " + color : ""}`}
                        ...
                        onClick={() => {
                          ...
                          logClick({ card: id, title, destination: path }, MODULES.TASK, { url: "/task-management" });
                          router.push(path);
                        }}
                      >
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                          <h4 className="text-sm lg:text-base font-semibold text-center text-slate-800 leading-tight mb-1 px-1 truncate group-hover:text-blue-600 transition-colors duration-200">{title}</h4>
          ...
          {/* Right: Charts */}
          <div ref={chartsContainerRef} id="charts-container" className="space-y-4 lg:space-y-3">
            {/* Pie Chart */}
            <motion.div
              className="bg-white/70 backdrop-blur-sm border border-slate-100/50 rounded-2xl p-9 lg:p-8 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-400 h-72 flex flex-col"
              initial={hasMounted ? { opacity: 0, scale: 0.95 } : false}
              ...
              whileHover={{ scale: 1.01 }}
            >
              <h3 className="text-base lg:text-lg font-semibold text-slate-800 mb-6 px-1">Task Distribution</h3>
              <div className="flex-1 flex items-center justify-center min-h-0">
                {taskStats.total > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={38} outerRadius={72} paddingAngle={2} stroke="white" strokeWidth={3}>
                        {pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                      </Pie>
                      <Tooltip content={<CustomPieTooltip />} />
                      <text x="50%" y="42%" textAnchor="middle" dominantBaseline="middle" fill="#475569" fontSize={12} fontWeight={600}>Total</text>
                      <text x="50%" y="52%" textAnchor="middle" dominantBaseline="middle" fill="#111827" fontSize={20} fontWeight={700}>{taskStats.total}</text>
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full text-center p-8">
                    <PieChartIcon size={40} className="text-slate-400 mb-4" strokeWidth={1.5} />
                    <p className="text-lg font-semibold text-slate-500 mb-2">No Data</p>
                    <p className="text-sm text-slate-500 max-w-xs">Start by creating tasks</p>
                  </div>
                )}
              </div>
            </motion.div>
            ...
            {/* Bar Chart */}
            <motion.div
              style={{ background: "rgba(255,255,255,0.7)", backdropFilter: "blur(8px)", border: "1px solid #f1f5f9", borderRadius: "16px", padding: "24px", height: "288px", display: "flex", flexDirection: "column", boxShadow: "0 8px 20px rgba(0,0,0,0.05)", transition: "all 0.4s ease" }}
              initial={hasMounted ? { opacity: 0, scale: 0.95 } : false}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              whileHover={{ scale: 1.01 }}
            >
              <div style={{ marginBottom: "14px" }}>
                <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#1e293b", marginBottom: "4px" }}>📈 Monthly Task Creation Trends</h3>
                <p style={{ fontSize: "13px", color: "#64748b", fontWeight: 500 }}>Each bar shows NEW tasks created each month (your department only)</p>
              </div>
              <div style={{ flex: 1 }}>
                {barData.some((d) => d.value > 0) ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={barData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }} barCategoryGap="25%">
                      ...
                      <Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={24}>
                        {barData.map((entry, index) => (
                          <Cell key={`bar-${index}`} fill={["#3b82f6","#60a5fa","#93c5fd","#bfdbfe","#dbeafe","#eff6ff","#e0f2fe","#bae6fd","#7dd3fc","#38bdf8","#0ea5e9","#0284c7"][index]} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center" }}>
                    <BarChart3 size={38} style={{ color: "#cbd5f5", marginBottom: "8px" }} strokeWidth={1.5} />
                    <p style={{ fontSize: "14px", fontWeight: 600, color: "#94a3b8", marginBottom: "4px" }}>No Trend Data</p>
                    <p style={{ fontSize: "12px", color: "#94a3b8" }}>Tasks need date fields for trends</p>
                  </div>
                )}
              </div>
            </motion.div>
