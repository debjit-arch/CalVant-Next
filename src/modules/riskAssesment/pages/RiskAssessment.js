...
import {
  ...
  logClick,
  MODULES,
} from "../../admin/shell/services/activities"; // ← adjust path to your actual activityService location
...
const RiskAssessment = () => {
  ...
  // ── LOG: Risk dashboard page load ─────────────────────────────────────────
  // Fires once after the component mounts and the user is confirmed.
  // We guard with `hasMounted` so it only runs client-side (Next.js SSR safe).
  useEffect(() => {
    if (!hasMounted || !user) return;
    logPageLoad("/risk/dashboard", MODULES.RISK);
  }, [hasMounted, user]);
  // ─────────────────────────────────────────────────────────────────────────
              ...
              {[
                ...
                <motion.div
                  ...
                  onClick={() => {
                    // ── LOG: stat card click ──────────────────────────────
                    logClick(
                      { filter: label.toLowerCase(), count: value },
                      MODULES.RISK,
                      { url: "/risk/dashboard" },
                    );
                      ...
                      show && (
                        <motion.div
                          ...
                          onClick={() => {
                            ...
                            logClick(
                              { card: id, title, destination: path },
                              MODULES.RISK,
                              { url: "/risk/dashboard" },
                            );
                            router.push(path);
