...
import Joyride, { STATUS } from "react-joyride";
import {
  Shield, ClipboardList, BarChart3, Clock,
  CheckCircle2, XCircle, Send, RefreshCw,
  BookOpen, UserCheck2, HelpCircle,
} from "lucide-react";
import {
...
const TPRMSection = () => {
  ...
  const {
    user, mounted, isRoot, isPrivilegedRole,
    isViewingManagedOrg, effectiveOrgId,
  } = useEffectiveOrg();
  ...
  const steps = [
    { target: "#tprm-header",      content: "Welcome to your Third Party Risk Management (TPRM) dashboard." },
    { target: "#stats-grid",       content: "Overview of assessments by their status (sent, submitted, approved, rejected)." },
    { target: "#action-cards",     content: "Manage TPRM questions, plan assessments, or view vendor reports." },
    { target: "#charts-container", content: "Visualize assessment distribution and trends analysis." },
  ];
  ...
  const adminActions = [
    { key: "questions", Icon: BookOpen,     title: "TPRM Questions", subtitle: "View & manage question bank",     color: "from-blue-400 to-blue-600",    onClick: () => setModal("questions") },
    { key: "conduct",   Icon: ClipboardList,title: "Plan TPRM",      subtitle: "Create & send assessments",       color: "from-violet-400 to-violet-600",onClick: () => setModal("conduct")   },
    { key: "report",    Icon: BarChart3,    title: "Vendor Report",  subtitle: "View scores & approve vendors",   color: "from-emerald-400 to-emerald-600",onClick: () => setModal("report")  },
  ];
    ...
    if (active && payload?.length) {
      ...
      return (
        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-lg">
          <p className="font-semibold text-slate-800 text-sm">{d.name}</p>
          <p className="text-2xl font-bold text-slate-900">{d.value}</p>
          <p className="text-xs text-slate-500">{stats.total ? ((d.value / stats.total) * 100).toFixed(1) : 0}% of total</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/50 to-indigo-50/30 flex flex-col overflow-hidden" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
        ...
        >
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-4 flex-1" style={{ justifyContent: "flex-start", textAlign: "left", alignItems: "flex-start" }}>
              ...
              <div style={{ textAlign: "left" }}>
                <h1 className="text-2xl font-semibold text-slate-800 leading-tight">Third Party Risk Management</h1>
                <p className="text-base text-slate-600 mt-1">
                  Root Dashboard · <span className="font-bold text-2xl text-slate-900">{stats.total}</span> total assessments
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-blue-100 text-blue-700">Root</span>
              <span className="text-sm font-semibold text-slate-600">{userName}</span>

              {/* LOG: refresh click */}
              <motion.button
                onClick={() => {
                  logClick({ action: "refresh_dashboard" }, MODULES.TPRM, { url: "/tprm" });
                  loadData();
                }}
                title="Refresh"
                className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors border border-slate-200 flex items-center justify-center"
              ...
              <motion.button
                className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-semibold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2"
                onClick={() => {
                  logClick({ action: "open_guide" }, MODULES.TPRM, { url: "/tprm" });
                  setRun(false);
                  setTimeout(() => setRun(true), 100);
                }}
            ...
            >
              {statCards.map(({ label, value, Icon, color }, i) => (
                <motion.div
                  ...
                  onClick={() => {
                    // LOG: stat card click
                    logClick({ filter: label.toLowerCase(), count: value }, MODULES.TPRM, { url: "/tprm" });
                    setModal("conduct");
                  }}
                >
                  ...
                  {adminActions.map(({ key, Icon, title, subtitle, color, onClick }, i) => (
                    <motion.div
                      ...
                      onClick={() => {
                        ...
                        logClick({ card: key, title, modal: key }, MODULES.TPRM, { url: "/tprm" });
                        onClick();
                      }}
                    >
                      ...
                      <p className="text-sm font-bold text-slate-800 mb-1 group-hover:text-blue-600 transition-colors">{title}</p>
                      <p className="text-xs text-slate-500">{subtitle}</p>
                    </motion.div>
                  ))}
                </AnimatePresence>
                ...
                {stats.total > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <text x="50%" y="55%" textAnchor="middle" dominantBaseline="middle" style={{ fill: "#1e293b", fontSize: 22, fontWeight: 800 }}>{stats.total}</text>
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              {barData.some((d) => d.value > 0) ? (
                <ResponsiveContainer width="100%" height="85%">
                  <BarChart data={barData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                    ...
                    <Bar dataKey="value" radius={[5, 5, 0, 0]} barSize={24}>
                      {barData.map((_, i) => (
                        <Cell key={i} fill={["#3b82f6","#60a5fa","#93c5fd","#bfdbfe","#dbeafe","#eff6ff","#e0f2fe","#bae6fd","#7dd3fc","#38bdf8","#0ea5e9","#0284c7"][i % 12]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
      ...
      {/* Modals */}
      {modal === "questions" && <TPRMQuestionsModal user={user} organization={org} onClose={() => setModal(null)} />}
      {modal === "conduct"   && <ConductTPRMModal   user={user} organization={org} onClose={() => setModal(null)} onSaved={loadData} />}
      {modal === "report"    && <TPRMReportModal    user={user} organization={org} onClose={() => setModal(null)} onRefresh={loadData} />}
    </div>
  );
};
